...
// CONFIG / API KEYS
// ═══════════════════════════════════════════
const GEMINI_API_KEY = 'AIzaSyBBRT2LLqNjJxA2Q7S06KTLo4MzrohZ5-A';
...
const stringsPerMpptIn   = document.getElementById('strings-per-mppt');
...
function updateInstalledPower() {
    if (!moduleParams || !moduleParams.pmax) {
        installedPowerEl.textContent = '— kWp';
        return;
    }
    const mpptCount      = parseInt(mpptInput.value) || 1;
    const modulesPerStr  = parseInt(modulesPerStringIn.value) || 1;
    const stringsPerMppt = parseInt(stringsPerMpptIn.value) || 1;
    const totalModules   = mpptCount * modulesPerStr * stringsPerMppt;
    const totalPower     = (totalModules * moduleParams.pmax / 1000).toFixed(2);
    installedPowerEl.textContent = `${totalPower} kWp`;
    installedPowerEl.title = `${totalModules} modułów × ${moduleParams.pmax} Wp`;
}
...
extractPdfBtn.addEventListener('click', async () => {
    const apiKey = localStorage.getItem('gemini_api_key') || openaiKeyInput.value.trim();
    ...
    try {
        // Extract text using pdf-parse (v1.1.1 — exports function directly)
        const pdfParse = require('pdf-parse');
        const buffer = fs.readFileSync(selectedPdfPath);
...
document.addEventListener('input', e => {
    const t = e.target;
    if (t === mpptInput || t === modulesPerStringIn || t === stringsPerMpptIn) {
        buildProtocolTable();
